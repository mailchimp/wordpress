...
<div class="wrap">
	...
	<div class="mailchimp-header">
		...
		<h1><?php esc_html_e( 'Mailchimp List Subscribe Form', 'mailchimp' ); ?> </h1>
	</div>
<?php
...
if ( ! $user || ! get_option( 'mc_api_key' ) ) {
	...
	<div>
		...
		<p class="mc-p">
			...
			printf(
				...
				esc_html( __( 'Don\'t have a Mailchimp account?', 'mailchimp' ) ),
				esc_html( __( 'Try one for Free!', 'mailchimp' ) )
			);
			?>
		</p>
	...
	if ( '' !== $user && isset( $user['username'] ) && $user['username'] ) {
		?>
<!--<div class="notes_msg">
		<strong><?php esc_html_e( 'Notes', 'mailchimp' ); ?>:</strong>
		<ul>
			<li><?php esc_html_e( 'Changing your settings at Mailchimp.com may cause this to stop working.', 'mailchimp' ); ?></li>
			<li><?php esc_html_e( 'If you change your login to a different account, the info you have setup below will be erased.', 'mailchimp' ); ?></li>
			<li><?php esc_html_e( 'If any of that happens, no biggie - just reconfigure your login and the items below...', 'mailchimp' ); ?></li>
		</ul>
</div>-->
		<?php
	}
		...
		if ( count( $lists ) === 0 ) {
			...
			<div class="error_msg">
				...
				printf(
					...
					esc_html( __( 'Uh-oh, you don\'t have any lists defined! Please visit %s, login, and setup a list before using this tool!', 'mailchimp' ) ),
...
<h3><?php esc_html_e( 'Subscribe Form Widget Settings for this List', 'mailchimp' ); ?>:</h3>
<h4><?php esc_html_e( 'Selected Mailchimp List', 'mailchimp' ); ?>: <?php echo esc_html( get_option( 'mc_list_name' ) ); ?></h4>
	<?php
}
		...
		<tr>
			<th colspan="4">
				...
				if ( ! is_array( $mv ) || count( $mv ) === 0 ) {
					?>
					<em><?php esc_html_e( 'No Merge Fields found.', 'mailchimp' ); ?></em>
					<?php
				} else {
					?>
			</th>
