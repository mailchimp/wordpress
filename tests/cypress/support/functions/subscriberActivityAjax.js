/**
 * Helpers for stubbing `mailchimp_sf_get_subscriber_activity` admin-ajax calls.
 * fetch() uses FormData (multipart); match by substring or parsed fields.
 *
 * @param {object} req Cypress intercepted request
 * @returns {boolean}
 */
function isSubscriberActivityRequest(req) {
	const action = 'mailchimp_sf_get_subscriber_activity';
	const { body } = req;
...
/**
 * Build a minimal successful subscriber-activity payload for stubs.
 *
 * @param {object} overrides Partial payload.data from PHP filter_and_aggregate.
